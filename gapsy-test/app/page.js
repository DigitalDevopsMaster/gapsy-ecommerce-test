...
export default function Home() {
  ...
  async function fetchNextPage() {
    ...
    try {
      ...
      const data = await response.json();
      const newItems = data.item.props.pageProps.initialData.searchResult?.itemStacks[0].items.filter((item) => item.price) || [];
  
      // Concatena los datos nuevos con los datos existentes
  ...
  return (
    <Page className="home-page">
      ...
      <ProductList data={data} className="item-list" nextPage={handleNext} onSearch={handleSearchItem} onEndReached={handleNext}/>
    </Page>
  ); 
}
